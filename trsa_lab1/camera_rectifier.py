...
class CameraRectifier(Node):
    ...
    def __init__(self):
        ...
        self.sub_image_raw = self.create_subscription(
            Image,
            "/camera/image_raw",
            self.camera_rectifier_callback,
            10
        )

        # Select the ost.yaml file to get the camera calibration parameters
        ost_file = os.path.join(ament_index_python.get_package_share_directory('trsa_lab1'), 'calibration','ost.yaml')
    ...
    def camera_rectifier_callback(self, msg):
        # Load the image and apply the Rectification
        cv_image = self.bridge.imgmsg_to_cv2(msg, "bgr8")
        cv_image_rect = self.bridge.imgmsg_to_cv2(msg, "bgr8")
